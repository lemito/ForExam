...
// пропускает пробелы и читает следующий значащий символ
char next_char() {
  char c = getchar();
  while (is_space(c))
    c = getchar();
...
Symbol * scan_operator(
    Symbol_type prev_type); // Для определения унарного минуса и обычного
Symbol * next_symbol() {
  // здесь будем хранить тип последнего считанного токена
  // надо это затем, чтобы, столкнувшись с унарным минусом не спутать его с
  // бинарным
  static Symbol_type prev_type = SYMBOL_NONE;

  Symbol * result;
  int c = next_char();
  if (c == EOF) { // Если считать новый ол
    ...
    // явная ошибка: не должно быть токенов, которые мы не могли бы разобрать
    result = malloc(sizeof(Symbol));
    result->type = SYMBOL_NONE;
  }

  // запоминаем в статической переменной тип только что считанного токена
...
Symbol * scan_number() {
  ...
  scanf("%lf", &temp->number); // читаем в структуру
  return (Symbol *)temp;
}
...
bool build_tree(Expression_node ** tree, Stack_symbol * rev) {
  ...
  if (symbol->type == SYMBOL_OP) {
    ...
    if (symbol_op->op == OP_UNARY_MINUS) {
      ...
      result = result && build_tree(&((*tree)->r), rev);
      // если result уже false, то build_tree уже не вызовется
...
Stack_symbol * Deikstra() {
  ...
  while ((symbol = next_symbol())->type != SYMBOL_NONE) {
    ...
    case SYMBOL_OP:
      ...
      // кладём новый оператор на верхушку стека
      Stack_symbol_push(temp, symbol);
      break;

    case SYMBOL_NUMBER:
    case SYMBOL_VAR:
      // эти вещи сразу пихаем в выходной стек
    ...
    case SYMBOL_RIGHT_BR:
      ...
      Stack_symbol_pop(temp); // Снимаем обнаруженную (
      break;
    }
  }
